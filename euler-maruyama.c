...
**/
    ...
    assert(0 == isnan(time));
    assert(2 == DIMENSION);
    rate_of_change.data[0] = 0.04 * point.data[0] * point.data[0] + 5 * point.data[0] + 140 - point.data[1] + IZHIKEVICH_I + ZIGGURAT_G * RNOR;
    rate_of_change.data[1] = IZHIKEVICH_A * (IZHIKEVICH_B * point.data[0] - point.data[1]);

    return rate_of_change;
}

struct Point em_step(
    struct Point s,
    long double size,
    int t
    )
{
    ...
    rate_of_change = ordinary_differential_equations(t, this_step);
    //x_{n+1} = x_n + rate_of_change 
    for(int i = 0; i < DIMENSION; i++){
        next_step.data[i] = this_step.data[i] + rate_of_change.data[i];
    }
    return next_step;
}
    ...
    )
{
    ...
    for(int t = 0; t < TIMESTEPS-1; t++){
        ...
        //normal step
        next_step = em_step(this_step, STEPSIZE, t);
        ...
        if(30.0 < next_step.data[0])
        {
            ...
            for(int j = 0; j < 100; j++)
            {
                ...
                next_step = em_step(this_step, this_step_size, t);
